...
async function api(path){
  const data=window.DUBVERSE_PREVIEW_DATA;
  const clean=path.split('?')[0];
  if(clean==='/api/settings') return structuredClone(data.settings);
  if(clean==='/api/projects') return structuredClone(data.projects.filter(p=>p.published).sort((a,b)=>(Number(b.featured)-Number(a.featured))||a.title.localeCompare(b.title,'es')));
  if(clean==='/api/studios') return structuredClone(data.studios.filter(s=>s.published).sort((a,b)=>a.name.localeCompare(b.name,'es')));
  if(clean.startsWith('/api/projects/')){
    const id=decodeURIComponent(clean.split('/').pop());
    const p=data.projects.find(x=>x.id===id&&x.published);
    if(!p) throw new Error('Proyecto no encontrado');
    const copy=structuredClone(p);
    copy.episodes=data.episodes.filter(e=>e.project_id===id&&e.published).sort((a,b)=>a.season-b.season||a.number-b.number);
    return copy;
  }
  if(clean.startsWith('/api/episodes/')){
    const id=decodeURIComponent(clean.split('/').pop());
    const e=data.episodes.find(x=>x.id===id&&x.published);
    if(!e) throw new Error('Episodio no encontrado');
    const copy=structuredClone(e);
    const p=data.projects.find(x=>x.id===e.project_id);
    copy.project=p?{id:p.id,title:p.title,poster:p.poster,banner:p.banner}:null;
    return copy;
  }
...
function about(){app.innerHTML=`<header class="page-header"><span class="eyebrow">Sobre Dubverse</span><h1>Una plataforma comunitaria, no un negocio.</h1></header><section class="section" style="padding-top:10px"><div class="about-panel"><p><strong>Dubverse organiza y presenta proyectos de fandoblaje realizados por sus respectivos equipos.</strong> La plataforma no incluye publicidad propia ni busca monetizar el material.</p><p>Los créditos, responsables y enlaces de cada proyecto se conservan en su ficha. El contenido audiovisual se aloja en proveedores externos, principalmente Archive.org.</p><p>Los titulares de derechos pueden solicitar la revisión o retiro de material mediante los canales de contacto del proyecto. La nueva arquitectura permitirá ocultar un episodio sin destruir el resto del catálogo.</p><p>La versión anterior estaba formada por numerosos HTML y carpetas. Esta publicación inicial genera el catálogo desde una única fuente de datos; la siguiente etapa conectará el panel administrativo y la base de datos.</p></div></section>`;}
