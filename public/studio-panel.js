...
function openProjectEditor(project) {
  ...
  const details = panelField('title', 'Título', project.title) + panelField('status', 'Estado', project.status, 'select', [{ value: 'UPCOMING', label: 'Próximamente' }, { value: 'ONGOING', label: 'En emisión' }, { value: 'FINISHED', label: 'Finalizado' }, { value: 'PAUSED', label: 'Pausado' }, { value: 'CANCELLED', label: 'Cancelado' }]) + panelField('projectDirector', 'Dirección del proyecto', project.projectDirector) + panelField('synopsis', 'Sinopsis', project.synopsis, 'textarea') + panelField('dubbingInfo', 'Información del doblaje', project.dubbingInfo, 'textarea') + panelField('credits', 'Créditos', project.credits, 'textarea') + panelField('published', 'Proyecto publicado', project.published, 'checkbox');
  const managed = `<div class="studio-managed-metadata"><strong>Metadatos gestionados por Dubverse</strong><p>Título original, títulos alternativos, alias de búsqueda, clasificación de edad y advertencias de contenido sólo pueden modificarse desde Administración.</p></div>`;
  showPanelEditor({ kind: 'project', id: project.id, title: 'Editar proyecto', media, fields: panelSection('Información', 'Metadata pública autorizada para el estudio.', details + managed) + panelSection('Imágenes', 'Selecciona archivos desde este dispositivo.', media.map(item => mediaField(item.key, item.label, item.url, item)).join('')) });
}
