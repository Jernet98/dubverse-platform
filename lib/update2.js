...
export function persistedArchiveNativeUrl(episode) {
  const identifier = String(episode.archive_identifier || episode.archiveIdentifier || '').trim();
  const raw = String(episode.archive_native_url || episode.archiveNativeUrl || '').trim();
  if (!identifier || !raw) return '';
  let parsed;
  try { parsed = new URL(raw); } catch { return ''; }
  if (parsed.protocol !== 'https:' || parsed.hostname !== 'archive.org' || parsed.search || parsed.hash) return '';
  ...
  let file;
  try {
    embeddedIdentifier = decodeURIComponent(parts[1] || '');
    file = parts.slice(2).map(decodeURIComponent).join('/');
  } catch { return ''; }
  if (embeddedIdentifier !== identifier || !file || file.includes('..')) return '';
  return archiveDownloadUrl(identifier, file);
}
...
export function episodePlayback(episode) {
  ...
  if (provider === 'ARCHIVE') {
    ...
    const requestedMode = String(episode.archive_playback_mode || episode.archivePlaybackMode || 'ARCHIVE_EMBED').toUpperCase();
    const verificationStatus = String(episode.archive_native_status || episode.archiveNativeStatus || 'UNVERIFIED').toUpperCase();
    const native = requestedMode === 'ARCHIVE_NATIVE_VERIFIED' && verificationStatus === 'NATIVE_OK'
      ? persistedArchiveNativeUrl(episode) : '';
    // Only the separately persisted verification may enable native playback;
    // an arbitrary .mp4 filename is never treated as proof of browser safety.
    return {
      provider, mode: native ? 'ARCHIVE_NATIVE_VERIFIED' : 'ARCHIVE_EMBED', status: native || fallback ? 'READY' : 'UNRESOLVED',
      identifier: String(identifier || '').trim(),
      file: String(file || '').trim(),
      source: native ? { kind: 'VIDEO', url: native } : null,
