...
export function episodePlayback(episode) {
  ...
  if (provider === 'ARCHIVE') {
    ...
    // A filename ending in .mp4 does not prove browser-safe codecs, a sensible
    // bitrate or useful byte-range behaviour. Archive's embed player can choose
    // the appropriate derivative, so it is the safe default until we persist
    // verified stream metadata for an episode.
    return { provider, source: null, fallback: fallback ? { kind: 'IFRAME', url: fallback, mode: 'ARCHIVE_EMBED' } : null };
  }
