...
export function archiveLegacyPlayback(value, expectedIdentifier = '') {
  const raw = String(value || '').trim();
  if (!raw) return null;
  let parsed;
  try { parsed = new URL(raw); } catch { return null; }
  const host = parsed.hostname.toLowerCase().replace(/^www\./, '');
  if (host !== 'archive.org' || !['http:', 'https:'].includes(parsed.protocol)) return null;
  let parts;
  try { parts = parsed.pathname.split('/').filter(Boolean).map(part => decodeURIComponent(part)); }
  catch { return null; }
  if (!['download', 'embed', 'details'].includes(parts[0]) || !parts[1]) return null;
  const identifier = parts[1];
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/.test(identifier) || (expectedIdentifier && identifier !== expectedIdentifier)) return null;
  const file = parts.length > 2 ? parts.slice(2).join('/') : '';
  if (file.includes('..')) return null;
  if (parts[0] === 'download' && file) {
    return { kind: 'VIDEO', url: archiveDownloadUrl(identifier, file), identifier, file };
  }
  return {
    kind: 'IFRAME',
    url: archiveEmbedUrlSafe(identifier, parts[0] === 'embed' ? file : ''),
    identifier,
    file: parts[0] === 'embed' ? file : ''
  };
}
...
export function episodePlayback(episode) {
  ...
  if (provider === 'ARCHIVE') {
    const storedIdentifier = String(episode.archive_identifier || episode.archiveIdentifier || '').trim();
    const storedFile = String(episode.archive_file || episode.archiveFile || '').trim();
    const legacy = archiveLegacyPlayback(episode.video_url || episode.videoUrl, storedIdentifier);
    const identifier = storedIdentifier || legacy?.identifier || '';
    const file = storedFile || legacy?.file || '';
    const direct = archiveDownloadUrl(identifier, file) || (legacy?.kind === 'VIDEO' ? legacy.url : '');
    const fileEmbed = archiveEmbedUrlSafe(identifier, file);
    const itemEmbed = archiveEmbedUrlSafe(identifier);
    const fallbacks = [];
    if (file && fileEmbed) fallbacks.push({ kind: 'IFRAME', url: fileEmbed, mode: 'ARCHIVE_FILE_EMBED' });
    if (legacy?.kind === 'IFRAME' && legacy.url !== fileEmbed && legacy.url !== itemEmbed) {
      fallbacks.push({ kind: 'IFRAME', url: legacy.url, mode: legacy.file ? 'ARCHIVE_FILE_EMBED' : 'ARCHIVE_ITEM_EMBED' });
    }
    if (itemEmbed && !fallbacks.some(candidate => candidate.url === itemEmbed)) {
      fallbacks.push({ kind: 'IFRAME', url: itemEmbed, mode: 'ARCHIVE_ITEM_EMBED' });
    }
    return {
      provider,
      source: direct ? { kind: mediaKind(direct), url: direct, mode: 'DIRECT_ARCHIVE' } : null,
...
export function mapPromo(row) {
  ...
  if (base.provider === 'YOUTUBE') base.playback = { kind: 'YOUTUBE', url: `https://www.youtube-nocookie.com/embed/${encodeURIComponent(base.providerIdentifier)}` };
    ...
    const direct = archiveDownloadUrl(base.providerIdentifier, base.providerFile);
    base.playback = { kind: direct ? 'VIDEO' : 'IFRAME', url: direct || archiveEmbedUrlSafe(base.providerIdentifier, base.providerFile), fallbackUrl: archiveEmbedUrlSafe(base.providerIdentifier, base.providerFile) };
