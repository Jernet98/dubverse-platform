...
export function episodePlayback(episode) {
  ...
  if (provider === 'ARCHIVE') {
    ...
    const direct = archiveDownloadUrl(identifier, file);
    const fallback = archiveEmbedUrlSafe(identifier, file);
    return { provider, source: direct ? { kind: mediaKind(direct), url: direct } : null, fallback: fallback ? { kind: 'IFRAME', url: fallback } : null };
  }
...
export function promoValue(raw, existing = {}) {
  ...
  let url = safeHttpUrl(raw.url ?? existing.url ?? '', 'La URL promocional');
  let providerIdentifier = String(raw.providerIdentifier ?? raw.provider_identifier ?? existing.providerIdentifier ?? existing.provider_identifier ?? '').trim();
  const providerFile = String(raw.providerFile ?? raw.provider_file ?? existing.providerFile ?? existing.provider_file ?? '').trim();
  if (provider === 'YOUTUBE') {
  ...
  if (provider === 'ARCHIVE' && !/^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/.test(providerIdentifier)) {
    throw new AppError(400, 'El identificador de Archive.org no es válido.');
  }
  ...
  const thumbnailUrl = safeHttpUrl(raw.thumbnailUrl ?? raw.thumbnail_url ?? existing.thumbnailUrl ?? existing.thumbnail_url ?? '', 'La miniatura');
