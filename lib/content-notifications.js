export function notificationImageUrl(row = {}) {
  const explicit = String(row.image_url || '').trim();
  if (explicit) return explicit;
  const isProject = row.target_type === 'PROJECT' || ['GLOBAL_NEW_PROJECT', 'STUDIO_NEW_PROJECT'].includes(row.type);
  if (!isProject) return '';
  return String(row.project_poster || row.project_banner || '').trim();
}
