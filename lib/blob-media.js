...
export function validatePanelImageSignature(bytes, contentType) {
  ...
  const webp = data.length >= 12 && String.fromCharCode(...data.slice(0, 4)) === 'RIFF'
    && String.fromCharCode(...data.slice(8, 12)) === 'WEBP';
  const valid = contentType === 'image/jpeg' ? jpeg : contentType === 'image/png' ? png : contentType === 'image/webp' ? webp : false;
  if (!valid) throw new AppError(400, 'El contenido del archivo no coincide con una imagen JPEG, PNG o WebP válida.');
  return true;
}
