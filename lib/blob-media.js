...
import { processImageBuffer } from './r2.js';
...
export function validatePanelMediaFile(file, kind) {
  ...
  if (!(file instanceof File)) throw new AppError(400, `Selecciona una imagen para el ${policy.label}.`);
...
export async function uploadPanelImage(file, { studioId, kind }) {
  if (!process.env.BLOB_READ_WRITE_TOKEN) throw new AppError(503, 'El almacenamiento de imágenes no está disponible en este entorno.');
  const { policy } = validatePanelMediaFile(file, kind);
  const bytes = Buffer.from(await file.arrayBuffer());
  const { output } = await processImageBuffer(bytes, policy.purpose);
  const studio = slugify(studioId) || 'studio';
  const filename = slugify(file.name.replace(/\.[^.]+$/, '')) || 'imagen';
  const blob = await put(`studios/${studio}/${policy.folder}/${Date.now()}-${filename}.webp`, output.data, {
    access: 'public',
    addRandomSuffix: true,
    contentType: 'image/webp'
  });
  return { url: blob.url, pathname: blob.pathname, width: output.info.width, height: output.info.height };
}
